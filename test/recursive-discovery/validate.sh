...
# Recursive Discovery Validation Script
...
setup_validation_environment "Recursive Discovery Validation"
    ...
    if [[ -f "$output_file" ]]; then
        ...
        if diff -q "$output_file" "$expected_file" >/dev/null; then
            ...
            if [[ "$filename" != *.log ]]; then
                ...
                echo "   Log file differs (timestamps expected)"
            fi
            differing_files=$((differing_files + 1))
        fi
    fi
...
if [[ $differing_files -eq 0 ]]; then
    echo -e "\n🎉 All recursive discovery tests passed!"
    exit 0
else
